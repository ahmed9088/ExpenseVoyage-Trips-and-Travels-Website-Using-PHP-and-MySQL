...
// Step 2: Fetch data from the 'agents' table
$query = "SELECT id, a_name, a_profetion, a_image, date_time FROM agent";
$result = $con->query($query);

// Step 3: Check if any results are returned
if ($result && $result->num_rows > 0) {
    $agents = $result->fetch_all(MYSQLI_ASSOC); // Fetch all rows as associative array
} else {
    $agents = []; // If no data, set $agents as an empty array
}
...
<div class="row">
<?php
// Step 4: Loop through the $agents array and display the data
if (!empty($agents)) {
    $counter = 0; // Counter to track items
    foreach ($agents as $agent) {
        if ($counter % 3 == 0 && $counter != 0) {
            // Close the previous row and start a new one after every third item
        ...
        <div class="col-lg-4 col-md-4 col-sm-12 pb-2">
            <div class="team-item bg-white mb-4">
                <div class="team-img position-relative overflow-hidden">
                    <img class="img-fluid w-100" style="aspect-ratio:2/3; height:258px;" src="admin/upload/agents/<?php echo htmlspecialchars($agent['a_image']); ?>" alt="<?php echo htmlspecialchars($agent['a_name']); ?>">
                ...
                <div class="text-center py-4">
                    <h5 class="text-truncate"><?php echo htmlspecialchars($agent['a_name']); ?></h5>
                    <p class="m-0"><?php echo htmlspecialchars($agent['a_profetion']); ?></p>
                </div>
            </div>
        </div>
    <?php
