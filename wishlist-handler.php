...
if (!isset($_SESSION['userid'])) {
    echo json_encode(['status' => 'error', 'message' => 'Please login to save trips.']);
    exit();
}
...
if ($trip_id <= 0) {
    echo json_encode(['status' => 'error', 'message' => 'Invalid trip reference.']);
    exit();
}
...
if ($action == 'toggle') {
    ...
    if (mysqli_num_rows($check) > 0) {
        ...
        echo json_encode(['status' => 'removed', 'message' => 'Trip removed from your collection.']);
    } else {
        mysqli_query($con, "INSERT INTO wishlist (user_id, trip_id) VALUES ($userid, $trip_id)");
        echo json_encode(['status' => 'added', 'message' => 'Trip added to your collection!']);
    }
}
?>
